...
from game.actions import DrawDeckAction, DrawDestinationAction, DrawFaceUpAction
...
class CFBaseAI(Player):
    ...
    Draw_Ticket_Threshold = 6  # the threshold of number of cars to draw ticket cards
    gui_debug = False
    ...
    def eval_path(self, path, all_paths, edge_costs, game):
        ...
        for card, num in cards_needed.items():
            if num <= 0:
                continue
            if card == Colors.none:
                ...
                cost += (num ** self.Edge_Color_Exp) # **: exponential power
    ...
    def debug_print(self, game):
        ...
        :return:
        """
        player_edge_claimed = game.get_edges_for_player(self) # a set of claimed edge names
        remaining_edges = self.path.edges - player_edge_claimed if self.path is not None else []
        return "Path:%s\n%s\nRemaining Edges: [%s]\nEdge Claimed: [%s]" \
