...
def find_paths_for_destinations(destinations, city_edges, max_cost, scoring=get_scoring(), player=None,
    ...
    for dest in dest_paths:
        ...
        if not all_paths:
            all_paths = dest_paths[dest] # The first round (destination) has no other paths to compare, so go straight to all_paths.
        ...
        # duplicates once in the new path, thus possibly reducing cost.
        else: # from the second round (destination), comparisons are made with previous paths in all_paths, new edges (if not already included in all_path) are added to all_paths
            ...
            for path1 in working_paths:
                for path2 in dest_paths[dest]:
                    ...
                    # In ideal cases, edges for different destinations can be merged, in other cases they are not,
                    # the 'paths' that are completely separate are also listed in all_paths, whether to select them depends on cost efficiency.
